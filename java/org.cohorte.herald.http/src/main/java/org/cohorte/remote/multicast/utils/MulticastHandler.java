 ...
 *      The Making of Talkie: Multi-interface broadcasting and multicast</a>
 */
public class MulticastHandler {
	...
	/** The socket to send heart beats */
	private DatagramChannel pSender;
	 ...
	 */
	public MulticastHandler(final IPacketListener aListener, final InetAddress aAddress, final int aPort) {
	...
	private NetworkInterface[] getMulticastInterfaces() throws SocketException {
		...
		final Enumeration<NetworkInterface> itfEnum = NetworkInterface.getNetworkInterfaces();
		...
		while (itfEnum.hasMoreElements()) {
			...
			try {
				...
				log(LogService.LOG_WARNING, "Error testing if an interface supports Multicast", ex);
			}
		}

		return multicastItfs.toArray(new NetworkInterface[multicastItfs.size()]);
	}
	 ...
	 *            Associated error
	 */
	private void log(final int aLevel, final String aMessage, final Throwable aThrowable) {
		...
		while (pThreadRun) {
			...
			try {
				...
				if (!(sender instanceof InetSocketAddress)) {
					...
					log(LogService.LOG_WARNING, "Unhandled kind of socket address: " + sender.getClass().getName());
					continue;
				}
				...
				if (pThreadRun && pListener != null && !pListener.handleError(ex)) {
	 ...
	 *             Error sending the packet
	 */
	public void send(final byte[] aData, final InetAddress aAddress, final int aPort) throws IOException {

		// Send the datagram
		pSender.send(ByteBuffer.wrap(aData), new InetSocketAddress(aAddress, aPort));
	}
	...
	private void setupMulticast() throws IOException {
		...
		try {
			...
			for (final NetworkInterface itf : getMulticastInterfaces()) {
				try {
					...
					log(LogService.LOG_ERROR, "Error joining multicast group on interface " + itf.getDisplayName(), ex);
				}
			}

			if (pJoinedGroups.isEmpty()) {
				// No success: abandon.
				throw new SocketException("No interface to join multicast group");
			}
		...
		pSender = DatagramChannel.open(family);
		pSender.setOption(StandardSocketOptions.SO_REUSEADDR, true);
	}
